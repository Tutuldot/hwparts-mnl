<div class="page-header d-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="page-title">Edit Sales Order Draft</h1>
        <p class="page-subtitle">Update items, quantity, or remarks for Draft Sales Order: <strong class="text-primary"><?= esc($order['so_number']) ?></strong></p>
    </div>
    <a href="<?= base_url('sales-orders/' . $order['id']) ?>" class="btn btn-outline-secondary btn-sm">
        <i class="fas fa-arrow-left me-1"></i> Back to Order Detail
    </a>
</div>
        ...
        <div class="card mb-3 shadow-sm border-0">
            ...
            <div class="card-body">
                    ...
                    <div class="input-group">
                        <select class="form-select" id="customerSelect" required>
                            ...
                            <?php foreach ($customers as $c): ?>
                                <option value="<?= $c['id'] ?>" data-terms="<?= $c['payment_terms'] ?>" data-wht="<?= $c['withholding_tax_rate'] ?? 1.00 ?>" <?= ($order['customer_id'] == $c['id']) ? 'selected' : '' ?>>
                ...
                <div class="mb-0">
                    ...
                    <textarea id="orderRemarks" class="form-control" rows="3" placeholder="Reference note, invoice remark, shipping codes, etc."><?= esc($order['remarks']) ?></textarea>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="row g-2 mb-3 bg-white p-2 rounded border">
                    <div class="col-6">
                        ...
                        <input type="number" step="0.01" min="0" max="100" id="vatRateInput" class="form-control form-control-sm" value="<?= number_format($order['vat_rate'] ?? 12.00, 2) ?>">
                    </div>
                    <div class="col-6">
                        <label class="form-label text-muted small mb-1">WHT Rate (%)</label>
                        <input type="number" step="0.01" min="0" max="100" id="whtRateInput" class="form-control form-control-sm" value="<?= number_format($order['withholding_tax_rate'] ?? 1.00, 2) ?>" title="Auto-populated from Customer database. You can manually override this rate.">
                    </div>
                </div>
                ...
                <button type="button" class="btn btn-primary w-100 py-2 font-weight-bold" id="checkoutBtn">
                    <i class="fas fa-save me-2"></i> Update Sales Order Draft
                </button>
            </div>
        </div>
    </div>
...
<script>
    // State of our POS cart, loaded from current db entries
    let cart = <?= $cartItems ?>;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Initial render of existing lines in cart
        renderCart();
    ...
    function updatePrice(partId, variantId, price) {
        const item = cart.find(i => i.part_id === partId && i.variant_id === variantId);
        if (item) {
            item.unit_price = parseFloat(price) || 0.00;
            if (item.unit_price < 0) item.unit_price = 0.00;
            renderCart();
        }
    }

    function updateDiscountType(partId, variantId, type) {
        const item = cart.find(i => i.part_id === partId && i.variant_id === variantId);
        if (item) {
    ...
    function updateDiscountVal(partId, variantId, val) {
        const item = cart.find(i => i.part_id === partId && i.variant_id === variantId);
        if (item) {
            item.discount_value = parseFloat(val) || 0;
    ...
    function computeLineDiscount(item) {
        ...
        if (item.discount_type === 'percent') {
            return roundPrice(gross * (item.discount_value / 100));
    ...
    function renderCart() {
        ...
        cart.forEach((item, index) => {
            ...
            tr.innerHTML = `
                ...
                <td class="text-center">
                    <button type="button" class="btn btn-xs btn-outline-danger btn-icon" onclick="removeFromCart(${item.part_id}, ${item.variant_id})">
    ...
    checkoutBtn.addEventListener('click', function() {
        ...
        checkoutBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Updating...';

        fetch('<?= base_url('sales-orders/' . $order['id'] . '/update') ?>', {
            method: 'POST',
            body: data
        })
        .then(res => res.json())
        .then(res => {
            if (res.status === 'success') {
                ...
                checkoutBtn.innerHTML = '<i class="fas fa-save me-2"></i> Update Sales Order Draft';
            }
        })
        .catch(err => {
            console.error(err);
